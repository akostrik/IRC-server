...
* не выключая сервер после предыдущего теста: один клиент соединяеися и создаёт канал, второй клиент соединяется и входит на канал, segfault

## ПРОТЕСТИРОВАНО на IRSSI 
* WHOIS alice (до регистрации) <== правильно отвечает. 
* TOPIC должна рассылать всем какое-то сообщение? <== TOPIC показывает тему всем пользователям отдельного канала, смена Топика = рассылка на участников канала его темы.
...
* NICK проверить, точно ли нужно :Nickname collision KILL  <== БАКЫТ - нужно по протоколу, проверка дубликатов работает.
* `TOPIC #channel` должно отвечать "недостаточно параметров" или устанавливать пустой топик? <=== должно выдавать текущий топик или его отсутствие, у нас это реализовано
